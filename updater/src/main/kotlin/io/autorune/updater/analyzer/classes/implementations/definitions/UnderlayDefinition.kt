...
class UnderlayDefinition : ClassAnalyzer() {
    ...
    override fun findClassNode(): ClassNode? {
        for (classNode in classPool) {
            ...
            val match = classNode.fields.count { it.desc == descInt && !Modifier.isStatic(it.access) } == 5
                    && classNode.fields.count { !Modifier.isStatic(it.access) } == 5
    ...
    private fun findFetchUnderlayDefinition()
    {

        val matches = AnalyzerSearching.searchAllClassesForMethod(listOf(), String.format("(I)L%s;", classNode.name))

        addMethodToClient("fetchUnderlayDefinition", matches[0].second.name, matches[0].first)
