...
class IdentityKitDefinition : ClassAnalyzer() {
    ...
    private fun findFetchIDKDefinition()
    {

        val matches = AnalyzerSearching.searchAllClassesForMethod(listOf(), String.format("(I)L%s;", classNode.name))

        addMethodToClient("fetchIdentityKitDefinition", matches[0].second.name, matches[0].first)
