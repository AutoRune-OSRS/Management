...
class InventoryDefinition : ClassAnalyzer() {
    ...
    override fun findClassNode(): ClassNode? {
        for (classNode in classPool) {
            ...
            if (classNode.interfaces.isNotEmpty() || Modifier.isAbstract(classNode.access))
                continue

            val match = classNode.fields.count { it.desc == descInt && !Modifier.isStatic(it.access) } == 1
                    && classNode.fields.count { !Modifier.isStatic(it.access) } == 1
                    && AnalyzerSearching.searchClassForMethod(classNode, listOf(ICONST_2)).size > 0
    ...
    private fun findFetchInventoryDefinition()
    {

        val matches = AnalyzerSearching.searchAllClassesForMethod(listOf(), String.format("(I)L%s;", classNode.name))

        addMethodToClient("fetchInventoryDefinition", matches[0].second.name, matches[0].first)
