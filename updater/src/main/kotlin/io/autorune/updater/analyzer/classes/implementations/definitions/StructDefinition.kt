...
@DependsOn(DoublyNode::class, IterableHashTable::class, RSByteBuffer::class)
class StructDefinition : ClassAnalyzer() {
    ...
    override fun findClassNode(): ClassNode? {
        for (classNode in classPool) {
            ...
            val match = classNode.fields.count { it.desc == String.format("L%s;", getClassAnalyser(IterableHashTable::class)?.classNode?.name)} == 1
    ...
    private fun findFetchStructDefinition()
    {

        val matches = AnalyzerSearching.searchAllClassesForMethod(listOf(), String.format("(I)L%s;", classNode.name))

        addMethodToClient("fetchStructDefinition", matches[0].second.name, matches[0].first)
