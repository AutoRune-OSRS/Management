...
class EnumDefinition : ClassAnalyzer() {
    ...
    override fun findClassNode(): ClassNode? {
        for (classNode in classPool) {
            ...
            val match = classNode.fields.count { it.desc == descString && !Modifier.isStatic(it.access) } == 1
                    && classNode.fields.count { it.desc == descStringArr && !Modifier.isStatic(it.access) } == 1
    ...
    private fun findFetchEnumDefinition()
    {

        val matches = AnalyzerSearching.searchAllClassesForMethod(listOf(), String.format("(I)L%s;", classNode.name))

        addMethodToClient("fetchEnumDefinition", matches[0].second.name, matches[0].first)
