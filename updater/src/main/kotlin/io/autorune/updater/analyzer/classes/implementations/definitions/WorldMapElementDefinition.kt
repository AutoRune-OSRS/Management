...
class WorldMapElementDefinition : ClassAnalyzer() {
    ...
    private fun findFetchWorldMapDefinition()
    {

        val matches = AnalyzerSearching.searchAllClassesForMethod(listOf(), String.format("(I)L%s;", classNode.name))

        addMethodToClient("fetchWorldMapDefinition", matches[0].second.name, matches[0].first)
