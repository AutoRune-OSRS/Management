...
class SequenceDefinition : ClassAnalyzer() {
    ...
    override fun findClassNode(): ClassNode? {
        for (classNode in classPool) {
            ...
            val match = classNode.fields.count { it.desc == descIntArr && !Modifier.isStatic(it.access) } == 5
    ...
    private fun findFetchSequenceDefinition()
    {

        val matches = AnalyzerSearching.searchAllClassesForMethod(listOf(), String.format("(I)L%s;", classNode.name))

        addMethodToClient("fetchSequenceDefinition", matches[0].second.name, matches[0].first)
